...
mod tests {
    ...
    #[test]
    fn test_parse_review_response_basic() {
        let text = r#"{"relevant": [1, 3], "refined_query": ""}"#;
        let outcome = parse_review_response(text, 3).unwrap();
        assert_eq!(outcome.relevant_indices, vec![0, 2]);
        assert_eq!(outcome.refined_query, "");
    }

    #[test]
    fn test_parse_review_response_with_refined_query() {
        let text = r#"{"relevant": [1], "refined_query": "better search terms"}"#;
        let outcome = parse_review_response(text, 5).unwrap();
        assert_eq!(outcome.relevant_indices, vec![0]);
        assert_eq!(outcome.refined_query, "better search terms");
    }

    #[test]
    fn test_parse_review_response_with_markdown_fences() {
        let text = "```json\n{\"relevant\": [2, 4], \"refined_query\": \"\"}\n```";
        let outcome = parse_review_response(text, 4).unwrap();
        assert_eq!(outcome.relevant_indices, vec![1, 3]);
    }

    #[test]
    fn test_parse_review_response_with_extra_text() {
        let text = "Here is my review:\n{\"relevant\": [1, 2, 3], \"refined_query\": \"\"}\nDone.";
        let outcome = parse_review_response(text, 3).unwrap();
        assert_eq!(outcome.relevant_indices, vec![0, 1, 2]);
    }

    #[test]
    fn test_parse_review_response_out_of_bounds_filtered() {
        let text = r#"{"relevant": [1, 5, 99], "refined_query": ""}"#;
        let outcome = parse_review_response(text, 3).unwrap();
        // Only index 1 (0-based 0) is valid; 5 and 99 are out of bounds.
        assert_eq!(outcome.relevant_indices, vec![0]);
    }

    #[test]
    fn test_extract_json_object() {
        assert_eq!(extract_json_object("hello {\"a\": 1} world"), Some("{\"a\": 1}"));
