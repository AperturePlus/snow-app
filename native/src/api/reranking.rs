...
mod tests {
    ...
    #[test]
    fn test_resolve_rerank_endpoint() {
        assert_eq!(
            resolve_rerank_endpoint(""),
            "https://api.jina.ai/v1/rerank"
        );
        assert_eq!(
            resolve_rerank_endpoint("https://api.example.com"),
            "https://api.example.com/v1/rerank"
        );
        assert_eq!(
            resolve_rerank_endpoint("https://api.example.com/v1"),
            "https://api.example.com/v1/rerank"
        );
        assert_eq!(
            resolve_rerank_endpoint("https://api.example.com/v1/rerank"),
            "https://api.example.com/v1/rerank"
        );
    }
    ...
    fn test_parse_rerank_response() {
        ...
        let result = parse_rerank_response(response, 3).unwrap();
        assert_eq!(result.len(), 3);
        assert_eq!(result[0].index, 2);
        assert!((result[0].score - 0.95).abs() < 1e-9);
    ...
    fn test_is_configured() {
        let config = RerankingConfig::from_settings("", "", "", 0, 0);
        assert!(!config.is_configured());

        let config = RerankingConfig::from_settings("model", "https://api.example.com", "key", 0, 0);
        assert!(config.is_configured());
    }
}
