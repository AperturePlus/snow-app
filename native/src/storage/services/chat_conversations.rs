...
/// Truncate conversation messages starting from the N-th user message (0-based).
/// Deletes that user message and everything after it in the conversation.
/// Frontend IDs differ from DB snowflake IDs, so we locate by user-message order.
pub fn truncate_chat_messages_from_user_index(
    database_path: &Path,
    conversation_id: &str,
    user_message_index: i32,
) -> Result<()> {
    let trimmed_conversation = conversation_id.trim();
    if trimmed_conversation.is_empty() {
    ...
    if user_message_index < 0 {
        return Err(Error::from_reason(
            "user_message_index must be >= 0".to_string(),
        ));
    }

    let mut connection = Connection::open(database_path).map_err(|error| {
        database::database_error(database_path, "truncate chat messages", error)
    })?;
    ...
    // Collect user-role message ids in order. Snowflake TEXT ids sort chronologically.
    let user_ids: Vec<String> = {
        ...
        let rows = statement
            .query_map(params![trimmed_conversation], |row| row.get(0))
            .map_err(|error| {
                database::database_error(database_path, "truncate chat messages", error)
            })?;
    ...
    let target_id = match user_ids.get(user_message_index as usize) {
        Some(id) => id.clone(),
        None => {
            // Target user message not yet persisted (e.g. only in UI memory).
            return Ok(());
        }
    };
    ...
    transaction
        .execute(
            ...
            params![trimmed_conversation, target_id],
        )
        .map_err(|error| {
            database::database_error(database_path, "truncate chat messages", error)
        })?;

    // Refresh conversation metadata after truncation.
    let message_count: i32 = transaction
        .query_row(
            "SELECT COUNT(*) FROM chat_messages WHERE conversation_id = ?1",
            params![trimmed_conversation],
            |row| row.get(0),
        )
        .map_err(|error| {
            database::database_error(database_path, "truncate chat messages", error)
    ...
    let last_preview: String = transaction
        .query_row(
    ...
    let last_response_id: String = transaction
        .query_row(
    ...
    transaction
        .execute(
            ...
            params![
                ...
                message_count,
                create_snippet(&last_preview, 180),
                last_response_id
            ],
        )
        .map_err(|error| {
