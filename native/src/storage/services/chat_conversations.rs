...
/// record when the conversation row does not exist or has no overrides.
                ...
                .query_row(
                    ...
                    |row| {
                        Ok(ConversationModes {
                            plan_mode: row.get::<_, Option<i64>>(0)?.map(|v| v != 0),
                            goal_mode: row.get::<_, Option<i64>>(1)?.map(|v| v != 0),
...
pub fn set_conversation_modes(
    ...
    database::open_connection(database_path)
        .and_then(|connection| {
            connection.execute(
                "INSERT INTO chat_conversations (conversation_id, plan_mode, goal_mode, goal_mode_token_budget)
                 VALUES (?1, ?2, ?3, ?4)
