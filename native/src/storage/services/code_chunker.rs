...
mod tests {
    ...
    fn test_basic_chunking() {
        ...
        // With 10 lines, max=4, overlap=1, step=3: [1-4], [4-7], [7-10]
    ...
    fn test_empty_content() {
        let chunks = chunk_content("", &ChunkingConfig::default());
        assert!(chunks.is_empty());
    }

    #[test]
    fn test_overlap_merging() {
        let content: String = (1..=5).map(|i| format!("line{i}")).collect::<Vec<_>>().join("\n");
        let config = ChunkingConfig {
        ...
        let chunks = chunk_content(&content, &config);
        // 5 lines, max=3, overlap=1, step=2: [1-3], [3-5]
