...
pub fn builtins() -> HashSet<&'static str> {
    [
        ...
        "fflush", "fgets", "fputs", "getchar", "putchar", "puts", "gets",
        "perror", "FILE", "stdin", "stdout", "stderr", "EOF",
        // stdlib
        ...
        "memmove", "memset", "memcmp", "strdup", "strndup",
        // constants
        "NULL", "true", "false", "EXIT_SUCCESS", "EXIT_FAILURE",
        // common
        "main", "argc", "argv", "const", "static", "extern", "register",
        "volatile", "inline", "restrict", "typedef", "struct", "union",
        "enum", "sizeof", "return", "if", "else", "for", "while", "do",
...
pub fn extract_definition(
    ...
    match kind {
        ...
        "init_declarator" => {
            let decl = node.child_by_field_name("declarator")?;
            let ident = find_nested_identifier(&decl, source)?;
            Some(make_def(ident.0, DefKind::Variable, ident.1, ident.2, depth))
        }
        "parameter_declaration" => {
            let decl = node.child_by_field_name("declarator")?;
            let ident = find_nested_identifier(&decl, source)?;
            Some(make_def(ident.0, DefKind::Parameter, ident.1, ident.2, depth))
        }
        _ => None,
    }
}
