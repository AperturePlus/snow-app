...
pub fn builtins() -> HashSet<&'static str> {
    [
        ...
        "sum", "abs", "round", "pow", "divmod", "hash", "id", "repr", "format",
        "input", "open", "iter", "next", "all", "any", "bin", "oct", "hex",
        "chr", "ord", "callable", "compile", "eval", "exec", "globals", "locals",
        "vars", "dir", "help", "memoryview", "bytearray", "bytes", "frozenset",
        "complex", "object", "slice", "ascii", "breakpoint", "__import__",
        // built-in constants
        ...
        "unittest", "pytest", "dataclasses", "enum", "copy", "string",
        "textwrap", "struct", "codecs", "csv", "sqlite3", "socket",
        "http", "urllib", "argparse", "configparser", "time", "random",
        ...
        "stat", "fileinput", "hashlib", "hmac", "secrets", "base64",
        "binascii", "difflib", "pprint", "uuid", "platform", "errno",
        "signal", "mmap", "ctypes", "array", "weakref", "types",
        "atexit", "gc", "select", "selectors", "queue", "sched",
        "_thread", "concurrent", "email", "html", "xml", "webbrowser",
        "cgi", "wsgiref", "xmlrpc", "ftplib", "poplib", "imaplib",
        "smtplib", "mailbox", "mimetypes", "encodings", "gettext",
        "locale", "calendar", "cmd", "shlex", "tkinter", "turtle",
        "curses", "readline", "rlcompleter", "pdb", "profile", "timeit",
        "trace", "distutils", "ensurepip", "venv", "site", "sysconfig",
        "builtins", "abc",
    ]
    .into_iter()
    .collect()
        ...
        "default_parameter" | "typed_parameter" | "typed_default_parameter" => {
            let name_node = node.child_by_field_name("name")
                .or_else(|| {
                    ...
                    let mut c = node.walk();
                    let found = node.named_children(&mut c).find(|ch| ch.kind() == "identifier");
                    found
                })?;
            let text = name_node.utf8_text(source.as_bytes()).ok()?.trim();
            if !text.is_empty() && text != "self" && text != "cls" {
