...
mod tests {
    ...
    #[test]
    fn parses_skill_frontmatter_and_allowed_tools() {
        let document = "---\nname: Test skill\ndescription: Test description\nallowed-tools:\n  - filesystem-read\n  - terminal-execute\n---\nUse these instructions.";
        let (metadata, content) = parse_skill_document(document).expect("skill should parse");
        ...
        assert_eq!(content, "Use these instructions.");
    }

    #[test]
    fn treats_only_boolean_false_as_disabled() {
        let document = "---\nname: Disabled skill\nenable: false\n---\nInstructions.";
        let (metadata, _) = parse_skill_document(document).expect("skill should parse");

        assert!(!metadata.enabled);
    }

    #[test]
    fn updates_or_appends_enable_frontmatter_field() {
        assert_eq!(
            update_frontmatter_enabled("name: Test\nenable: true\n", false, "\n"),
            "name: Test\nenable: false\n"
        );
        assert_eq!(
            update_frontmatter_enabled("name: Test\n", false, "\n"),
    ...
    #[test]
    fn normalizes_cross_platform_skill_ids() {
        assert_eq!(normalize_skill_id(".\\group\\skill"), "group/skill");
    }
}
