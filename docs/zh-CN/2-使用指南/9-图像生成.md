...
| `images` | array | 参考图 `[{data, mimeType}]` 或 `[{path, mimeType}]`，图生图/编辑；`path` 为 upload 目录下的相对路径（文本化消息中的 `[Reference image #N ...]` 引用块，服务端自动读取）；**服务端上限 14 张**、单张 ≤20MB（工具描述引导 AI 单次 ≤5 张，以兼容部分服务商更严格的限制） |
