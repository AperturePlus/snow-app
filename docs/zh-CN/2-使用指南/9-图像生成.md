...
（OpenAI 兼容 / Gemini 可混配，多个渠道可同时启用），点击
「添加渠道」创建：
...
- **多图生成**：可要求一次生成多张（工具参数 `n` 单次最多 4 张）。
  AI 同时发起多个生成请求时**并行执行**，最多同时进行
  **设置里的「最大并发生成数」**（1-8，默认 4）个，超出部分自动排队，
  完成一张补一张，界面卡片会实时显示各自的生成进度；
...
| `images` | array | 参考图 `[{data, mimeType}]` 或 `[{path, mimeType}]`，图生图/编辑；`path` 为 upload 目录下的相对路径（文本化消息中的 `[Reference image #N ...]` 引用块，服务端自动读取），最多 14 张、单张 ≤20MB |
...
| `background` | enum | OpenAI：`opaque`（默认）/ `transparent` / `auto`；模型不支持透明背景（如 `gpt-image-2`）时自动降级为 `opaque` |
...
| 操作 | 示例 |
| --- | --- |
| 查看渠道状态 | `config-list` + `scope: "imagegen"` → 返回各渠道的 enabled / model / configured，以及全局 `maxConcurrentImages`（最大并发生成数） |
| 读取配置 | `config-get` + `scope: "imagegen"` + `key: "openai"`（省略 key 返回完整设置）；读取并发数用 `key: "maxConcurrentImages"` |
| 写入配置 | `config-set` + `scope: "imagegen"` + `value: {openai: {...}}`（部分更新自动合并，未提供的字段保留原值）；单独调整并发数用 `value: {maxConcurrentImages: 6}`（自动收敛到 1-8） |
...
| 一次生成多张时如何控制并发 | 设置 → 图像生成 →「最大并发生成数」（1-8，默认 4）；超出的请求自动排队，完成一张补一张；服务商报 429 限流时调低该值 |
